...
func TestHeapSort(t *testing.T) {
	actual, err := HeapSort(testDataIntAsc.elems, testDataIntAsc.sd, utils.IntComparator)
	if err != nil {
		t.Errorf("error while sorting %v, error was: %v", testDataIntAsc.elems, err)
	}
	assert.Equal(t, testDataIntAsc.expected, actual)

	actual, err = HeapSort(testDataIntDesc.elems, testDataIntDesc.sd, utils.IntComparator)
	if err != nil {
		t.Errorf("error while sorting %v, error was: %v", testDataIntDesc.elems, err)
	}
	assert.Equal(t, testDataIntDesc.expected, actual)

	actual, err = HeapSort(testDataIntAscRepeatElem.elems, testDataIntAscRepeatElem.sd, utils.IntComparator)
	if err != nil {
	...
	_, err = HeapSort([]string{}, Ascending, utils.StringComparator)
